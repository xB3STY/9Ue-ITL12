...
class UserModel
{
    ...
    */
    ...
    public static function doesEmailAlreadyExist($user_email)
    {
        // Wenn die E-Mail leer ist, sofort false zurückgeben
        if (empty($user_email)) {
            return false;
        }
    ...
    public static function getAllUsersWithGroups()
    {
        ...
        $sql = "SELECT users.user_id, users.user_name, users.user_email, user_groups.group_name 
                FROM users 
    ...
    public static function updateUserGroup($user_id, $group_id)
    {
        ...
        // Optional: Mappe die group_id auf einen passenden user_account_type
        $accountType = $group_id == 1 ? 2 : ($group_id == 2 ? 3 : 7); // Beispiel: 1=Gast, 2=User, 7=Admin

        $sql = "UPDATE users SET group_id = :group_id, user_account_type = :account_type WHERE user_id = :user_id";
        $query = $db->prepare($sql);
